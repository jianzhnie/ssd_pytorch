...
if __name__ == '__main__':
    import numpy as np
    labels  = np.random.choice(6,6)
    truths = torch.from_numpy(np.random.rand(24).reshape((6,4)))
    overlaps = torch.from_numpy(np.random.rand(48).reshape((6,8)))
    best_prior_overlap, best_prior_idx = overlaps.max(1, keepdim=True)
